...
export function ClientPJStep1({ form }: { form: UseFormReturn<ClientPJFormValues> }) {
  ...
  async function handlePreencherDados() {
    setLoadingCnpj(true)
    const result = await lookupCnpj(form.getValues('cnpj') ?? '')
    form.setValue('razaoSocial', form.getValues('razaoSocial') || result.razaoSocial)
    form.setValue('nomeFantasia', form.getValues('nomeFantasia') || result.nomeFantasia)
    form.setValue('telefone', form.getValues('telefone') || result.telefone)
    form.setValue('email', form.getValues('email') || result.email)
