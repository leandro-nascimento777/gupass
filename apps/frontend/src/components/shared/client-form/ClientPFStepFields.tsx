...
export function ClientPFStep1({ form, hiddenFields = [] }: ClientPFStep1Props) {
  ...
  async function handlePreencherDados() {
    setLoadingCpf(true)
    const result = await lookupCpf(form.getValues('cpf') ?? '')
    form.setValue('nomeCompleto', form.getValues('nomeCompleto') || result.nomeCompleto)
    form.setValue('dataNascimento', form.getValues('dataNascimento') || result.dataNascimento)
    form.setValue('sexo', form.getValues('sexo') || result.sexo)
    setLoadingCpf(false)
  }
...
export function ClientEnderecoStep<T extends { cep?: string; bairro?: string; logradouro?: string; numero?: string; complemento?: string; cidade?: string; estado?: string }>({
  ...
  async function handleCepBlur() {
    ...
    if (result) {
      form.setValue('bairro' as never, result.bairro as never)
      form.setValue('logradouro' as never, result.logradouro as never)
      form.setValue('cidade' as never, result.cidade as never)
      form.setValue('estado' as never, result.estado as never)
    }
    setLoadingCep(false)
  }
